...
#ifdef ENABLE_SIGINT
static void UI_DisplaySigintMain(void)
{
	char String[22];
	...
	if (FUNCTION_IsRx() || gScanStateDir != SCAN_OFF || SCANNER_IsScanning())
		vfo_num = gEeprom.RX_VFO;
	...
	if (gScanStateDir != SCAN_OFF || SCANNER_IsScanning())
		UI_PrintStringSmallBold("K5-SIG SCAN", 2, 0, 0);
	else if (FUNCTION_IsRx())
		UI_PrintStringSmallBold("K5-SIG RX", 2, 0, 0);
	else if (gCurrentFunction == FUNCTION_TRANSMIT)
		UI_PrintStringSmallBold("K5-SIG TX", 2, 0, 0);
	else
		UI_PrintStringSmallBold("K5-SIG MON", 2, 0, 0);

	if (IS_MR_CHANNEL(gEeprom.ScreenChannel[vfo_num])) {
		char Name[17];
		SETTINGS_FetchChannelName(Name, gEeprom.ScreenChannel[vfo_num]);
		if (Name[0] != 0)
			UI_PrintStringSmallNormal(Name, 73, 0, 0);
		else {
			sprintf(String, "CH-%03u", gEeprom.ScreenChannel[vfo_num] + 1);
			UI_PrintStringSmallNormal(String, 86, 0, 0);
		}
	} else if (IS_FREQ_CHANNEL(gEeprom.ScreenChannel[vfo_num])) {
		sprintf(String, "VFO-%u", 1 + gEeprom.ScreenChannel[vfo_num] - FREQ_CHANNEL_FIRST);
		UI_PrintStringSmallNormal(String, 94, 0, 0);
	}
	...
	if (gInputBoxIndex > 0 && IS_FREQ_CHANNEL(gEeprom.ScreenChannel[vfo_num]) && gEeprom.TX_VFO == vfo_num) {
		...
		if (frequency < _1GHz_in_KHz) {
			UI_PrintStringSmallNormal(String + 7, 106, 0, 3);
			String[7] = 0;
	...
	const char *mode = gModulationStr[vfoInfo->Modulation];
	const char *bw = (vfoInfo->CHANNEL_BANDWIDTH == BANDWIDTH_NARROW) ? "NAR" : "WIDE";
	const char pwr_list[][2] = {"L", "M", "H"};
	const char *pwr = pwr_list[vfoInfo->OUTPUT_POWER % 3];
	sprintf(String, "%s  %s  PWR:%s", mode, bw, pwr);
	UI_PrintStringSmallNormal(String, 2, 0, 5);

	if (VfoState[vfo_num] != VFO_STATE_NORMAL && VfoState[vfo_num] < ARRAY_SIZE(VfoStateStr))
		UI_PrintStringSmallBold(VfoStateStr[VfoState[vfo_num]], 2, 0, 6);
	else if (gScanStateDir != SCAN_OFF || SCANNER_IsScanning()) {
#ifdef ENABLE_SIGINT
		const char *preset = SIGINT_GetActiveScanPresetName();
		if (preset != NULL)
			UI_PrintStringSmallNormal(preset, 2, 0, 6);
		else
#endif
			UI_PrintStringSmallNormal("SCAN ACTIVE", 2, 0, 6);
	}
	else
		UI_PrintStringSmallNormal(SIGINT_GetLastBankMessage(), 2, 0, 6);
